...
public class SubwayManagerImpl implements SubwayManager
{
        ...
        throws SubwayException
    {
		...
		int exitTimeNum = Integer.valueOf(enterTime.replaceAll("[:]", ""));
		...
		// E05:无效的时间关系
		if(Integer.valueOf(enterTimeArr[0]) > 24 || Integer.valueOf(exitTimeArr[0]) > 24 ||
				Integer.valueOf(enterTimeArr[1]) > 60 || Integer.valueOf(exitTimeArr[1]) > 60 ||
				exitTimeNum <= enterTimeNum) {
			throw new SubwayException(ReturnCodeEnum.E05, card);
		}
		
		//  计算乘车总费用
		int billing = BillingCounter.calculateBilling(card.getCardType(), enterStation, null, exitStation, null);
		...
		if(card.getMoney() < billing ) {		
			...
			consumeRecord.setConsumeMoney(billing);
			consumeRecord.setEnterStation(enterStation);
			consumeRecord.setEnterTime(enterTime);
			consumeRecord.setExitStation(exitStation);
			consumeRecord.setExitTime(exitTime);
			
			// 记录消费记录
			cardInfoStorage.saveConsumeRecordById(cardId, consumeRecord);
			
			// 扣费
			return cardManager.consume(cardId, billing);
		}
    }
